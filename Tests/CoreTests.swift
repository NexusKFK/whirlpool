...
func runCoreTests() throws {
    ...
    let clamped = try JSONDecoder().decode(TickerConfig.self, from: Data("{\"scrollSpeed\":0,\"boardRefresh\":-1,\"defaultWidth\":1000,\"boardOrigin\":[1],\"language\":\"unknown\"}".utf8))
    precondition(clamped.scrollSpeed == 0.02 && clamped.boardRefresh == 5 && clamped.defaultWidth == 60 && clamped.boardOrigin == nil && clamped.language == "system")
