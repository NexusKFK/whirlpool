...
配色跟随所在显示面的明暗：暗色菜单栏上代码/价格为白色，亮色菜单栏上为近黑色；红绿涨跌色在亮底自动换成对比度达标的深色版（**显示 → 配色**：自适应、单色、琥珀、绿色）。浮动行情条可选毛玻璃胶囊背板，在亮色窗口上也清晰可读。鼠标悬停在行情条上会暂停滚动；双击报价卡某一行（或菜单 **打开图表**）打开 TradingView / Yahoo 图表。

滚动由 GPU 合成：每轮行情只绘制一次，Core Animation 按屏幕刷新率推进，滚动时 app 自身 CPU 接近 0%。

Windows：直接运行 `Whirlpool.exe`（免安装）。右键任务栏通知区域图标、行情条或报价卡弹出菜单，双击图标打开设置；Ctrl + 单击行情条切换自选池，双击报价卡某行打开图表。配置在 `%APPDATA%\Whirlpool\config.json`（Pinwheel 时期的配置会自动复制过来一次）。
...
在 **显示 → 显示器** 里选择浮动行情条和报价卡显示在哪块屏幕（菜单栏跑马灯会由 macOS 同步到每块屏幕的菜单栏，宽度按所选屏幕计算）。**锁定浮窗位置** 防止误拖，**浮动行情条点击穿透** 让鼠标直接点到下面的窗口——两者都在菜单栏图标的右键菜单和设置里。

可建多套自选池，在菜单 **自选池**、⌥ 单击行情条或 `whirlpool --list 名称` 快速切换。价格小数位按品种自动决定（外汇 4 位、A 股 ETF 3 位、低价加密资产更多），也可在自选页逐个指定。
