...
- `Windows/Pinwheel.Core/`: Windows models, quote client, localization, and price rules; runs on any .NET 10 host.
- `Windows/Pinwheel.Windows/`: Windows Forms UI and tray integration.
...
- `Tests/` and `Windows/Pinwheel.Core.Tests/`: deterministic regressions using fixtures, no live market requests.

Run `bash tools/test-price-flash.sh` on macOS and `dotnet run --project Windows/Pinwheel.Core.Tests -c Release` on any .NET 10 host. Run `Pinwheel.exe --smoke-test` on Windows to instantiate both languages and render the ticker. This is a smoke test, not a substitute for the Windows manual checklist.
...
3. Build macOS with `bash build-app.sh --arch arm64 --arch x86_64` and verify `codesign --verify --deep --strict Pinwheel.app`.
