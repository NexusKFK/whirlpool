...
parts = [(size, (root / f'icons/whirlpool.iconset/icon_{size}x{size}.png').read_bytes()) for size in [16, 32, 128, 256]]
...
for size, data in parts:
    ...
    offset += len(data)
(root / 'icons/whirlpool.ico').write_bytes(output + b''.join(data for _, data in parts))
